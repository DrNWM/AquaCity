...
export default class Stage04 extends Phaser.Scene {
    ...
    create() {
        this.budget = 20.00 + this.carryoverBudget;
        this.budgetUI = null;
        ...
        // Low Pressure Source (visual cue)
        const sourceSprite = this.add.sprite(
        ...
        sourceSprite.setAlpha(0.6); // Look weaker
        ...
        // UI Headers
        this.add.text(200, 20, 'Stage 04: The Dry Season', {
    ...
    simulateFlow() {
        ...
        // Start with 5 pressure
        const queue = [{ x: this.sourcePos.x, y: this.sourcePos.y, waterType: 'clean', pressure: 5 }];
        ...
        if (targetsReached === this.targetPos.length) {
            this.triggerWin();
             ...
             const msg = this.add.text(400, 300, 'LOW PRESSURE! USE A PUMP.', {
                font: '32px Arial', fill: '#ff0000', backgroundColor: '#000000', padding: {x: 10, y: 10}
            }).setOrigin(0.5).setDepth(20);
    ...
    triggerWin() {
        ...
        nextBtn.on('pointerdown', () => {
            this.scene.start('Stage05', { budget: nextBudget }); // Final stage
        });
    }
}
